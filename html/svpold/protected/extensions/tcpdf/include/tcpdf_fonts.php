...
class TCPDF_FONTS {
	...
	public static function UTF8ArrayToUniArray($ta, $isunicode=true) {
		if ($isunicode) {
			return array_map(array('TcPdf\TCPDF_FONTS', 'unichrUnicode'), $ta);
		}
		return array_map(array('TcPdf\TCPDF_FONTS', 'unichrASCII'), $ta);
	}
	...
	public static function UTF8StringToArray($str, $isunicode=true, &$currentfont) {
		if ($isunicode) {
			...
			$carr = array_map(array('TcPdf\TCPDF_FONTS', 'uniord'), $chars);
		} else {
			$chars = str_split($str);
