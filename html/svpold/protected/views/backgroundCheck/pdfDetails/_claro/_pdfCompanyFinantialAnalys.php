...
if ($backgroundCheck->getVerificationSection(VerificationSectionType::DETAIL_COMPANY_FINANTIAL_ANALYS)->percentCompleted >=100 ) {
    ...
    $pdf->SetFillColor(255,255,255); $pdf->SetTextColor(0,0,0);
    $pdf->MultiCell(80, 0, "Cobertura Gastos Financieros" , 1, 'L', 1, 0, '', '', true);
    $pdf->MultiCell(45, 0, number_format($coberturaGastosFinancieros_0, 2, '.', ''). "%" , 1, 'R', 1, 0, '', '', true);
    $pdf->MultiCell(45, 0, number_format($coberturaGastosFinancieros_1, 2, '.', ''). "%" , 1, 'R', 1, 1, '', '', true);

    $pdf->SetFillColor(150,150,150); $pdf->SetTextColor(0,0,0);
    $pdf->MultiCell(170, 0, "Indicadores Actividad" , 1, 'L', 1, 1, '', '', true);

    $pdf->SetFillColor(255,255,255); $pdf->SetTextColor(0,0,0);
    $pdf->MultiCell(80, 0, "Endeudamiento/Ventas" , 1, 'L', 1, 0, '', '', true);
    $pdf->MultiCell(45, 0, number_format($endeudamientoVentas_0, 2, '.', ''). " veces\n" , 1, 'R', 1, 0, '', '', true);
    $pdf->MultiCell(45, 0, number_format($endeudamientoVentas_1, 2, '.', ''). " veces\n" , 1, 'R', 1, 1, '', '', true);
    
    $pdf->SetFillColor(255,255,255); $pdf->SetTextColor(0,0,0);
    $pdf->MultiCell(80, 0, "Cobertura de Gastos No Oper." , 1, 'L', 1, 0, '', '', true);
    ...
    $pdf->SetFillColor(255,255,255); $pdf->SetTextColor(0,0,0);
    $pdf->MultiCell(80, 0, "Quebranto Patrimonial" , 1, 'L', 1, 0, '', '', true);
    ...
    $pdf->SetFillColor(255,255,255); $pdf->SetTextColor(0,0,0);
    $pdf->MultiCell(80, 0, "Fondo de Liquidez" , 1, 'L', 1, 0, '', '', true);
    ...
    $pdf->MultiCell(125, 0, "Capacidad de Contratación" , 1, 'L', 1, 0, '', '', true);
    $pdf->SetFillColor(255,255,255);
    $pdf->MultiCell(45, 0, $sCFA->kContratacion. " S.M.M.L.V." , 1, 'L', 1, 1, '', '', true);
    
    $pdf->SetFillColor(230);
    $pdf->MultiCell(125, 0, "Capacidad de Ejecución" , 1, 'L', 1, 0, '', '', true);
    $pdf->SetFillColor(255,255,255);
    $pdf->MultiCell(45, 0, $sCFA->kEjecucion , 1, 'R', 1, 1, '', '', true);
    
    if( $claroProveedoresNacionales == TRUE){
        $pdf->SetFillColor(230);  $pdf->SetTextColor(0,0,0); $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(125, 0, "Presenta algún tipo de Liquidación" , 1, 'L', 1, 0, '', '', true);
        $pdf->SetFillColor(255,255,255);
        $pdf->MultiCell(45, 0, $XMLQuestionResult['adicionalClaro_1'] , 1, 'C', 1, 1, '', '', true);
    
        if($XMLQuestionResult['adicionalClaro_1_comment'] != ""){
            $pdf->SetFillColor(255,255,255); $pdf->SetFont('Arial', 'I', 7);
            $pdf->MultiCell(170, 0, "Comentarios: " . $XMLQuestionResult['adicionalClaro_1_comment'] , 1, 'L', 1, 1, '', '', true);
        }
        
        $pdf->SetFillColor(230);  $pdf->SetTextColor(0,0,0); $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(125, 0, "Reorganización Empresarial (LEY 1116 - 550)" , 1, 'L', 1, 0, '', '', true);
        $pdf->SetFillColor(255,255,255);
        $pdf->MultiCell(45, 0, $XMLQuestionResult['adicionalClaro_2'] , 1, 'C', 1, 1, '', '', true);
    
        if($XMLQuestionResult['adicionalClaro_2_comment'] != ""){
            $pdf->SetFillColor(255,255,255); $pdf->SetFont('Arial', 'I', 7);
            $pdf->MultiCell(170, 0, "Comentarios: " . $XMLQuestionResult['adicionalClaro_2_comment'] , 1, 'L', 1, 1, '', '', true);
        }
    }
